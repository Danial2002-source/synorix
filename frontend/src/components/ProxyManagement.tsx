...
const ProxyManagement: React.FC = () => {
  ...
  const fetchLogs = async () => {
    try {
      ...
      const transformedLogs = (data.data || []).map((log: any) => ({
        ...
        method: log.method,
        url: log.url,
        status: log.response_status || log.status_code || log.status,
        // Convert response_time from seconds to milliseconds
        latency: (log.response_time || log.latency) ? 
                 (log.response_time ? log.response_time * 1000 : log.latency) : 
                 0,
        client_ip: log.client_ip || log.ip,
  ...
  const fetchLatencyData = async () => {
    try {
      const response = await fetch(`/api/proxy/latency`);
                    ...
                    <div className="latency-stats">
                      ...
                      <div className="latency-stat">
                        ...
                        <span className="latency-value">
                          {logs.length > 0 && logs.some(l => l.latency > 0)
                            ? Math.min(...logs.filter(l => l.latency > 0).map(l => l.latency)).toFixed(0)
                            : 0}ms
                        </span>
                      </div>
                      <div className="latency-stat">
                        ...
                        <span className="latency-value">
                          {logs.length > 0 && logs.some(l => l.latency > 0)
                            ? Math.max(...logs.filter(l => l.latency > 0).map(l => l.latency)).toFixed(0)
                            : 0}ms
                        </span>
                      </div>
                    </div>
