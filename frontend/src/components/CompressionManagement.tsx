            ...
            <div className="overview-tab">
              <div className="overview-grid">
                ...
                <div className="overview-card">
                  ...
                  <div className="compression-impact">
                    {logs.length > 0 ? (
                      <>
                        <div className="impact-row">
                          <span className="impact-label">Before Compression:</span>
                          <span className="impact-value before">
                            {formatBytes(logs.reduce((sum, log) => sum + log.original_size, 0))}
                          </span>
                        </div>
                        <div className="impact-arrow">→</div>
                        <div className="impact-row">
                          <span className="impact-label">After Compression:</span>
                          <span className="impact-value after">
                            {formatBytes(logs.reduce((sum, log) => sum + log.compressed_size, 0))}
                          </span>
                        </div>
                        <div className="impact-savings">
                          <TrendingDown size={16} />
                          <span>
                            Saved {formatBytes(logs.reduce((sum, log) => sum + (log.original_size - log.compressed_size), 0))}
                            {' '}({formatRatio(logs.reduce((sum, log) => sum + log.compression_ratio, 0) / logs.length)})
                          </span>
                        </div>
                      </>
                    ) : (
